let db = openDatabase("laba5", "1.0", "Lab5DB", 2 * 1024 * 1024);

db.transaction(function (t) {
     t.executeSql("CREATE TABLE IF NOT EXISTS groups (number INTEGER NOT NULL, course TEXT NOT NULL, faculty TEXT NOT NULL, quantity INTEGER NOT NULL, email TEXT NOT NULL, phone TEXT, address TEXT, curator TEXT)");
     //t.executeSql("DROP table groups");
});

class Group {

    constructor(group_number, course, faculty, student_quantity, email) {
...
function addGroup() {
    ...
    let group = new Group(parseInt(number, 10), course, faculty, parseInt(quantity, 10), email);
...
function insert(group, number) {
    db.transaction(function (t) {
        t.executeSql("INSERT INTO groups (number, course, faculty, quantity, email) VALUES (?,?,?,?,?)",
            [group.group_number, group.get_course, group.get_faculty, group.get_student_quantity, group.get_email]);
    });
    if("_phone" in group) {
        db.transaction(function (t) {
...
function loadTable() {
    if (db) {
        db.transaction(function (t) {
            t.executeSql("SELECT rowid, number, course, faculty, quantity, email, phone, address, curator FROM groups",
                [], updateTable);
        })
    }
}
...
function alertNumber(transaction, result) {
    let rows = result.rows;
    let message = "Список групп, где меньше 20 студентов\n";
    for (let i = 0; i < rows.length; i++) {
        let row = rows.item(i);
        message += row.number;
        message += ": ";
...
function addNewProperties() {
    ...
    div.insertAdjacentHTML("beforeend", "<select id=\"addInfo\" name=\"newProperty\"><option value=\"phone\">Телефон</option><option value=\"address\">Адрес деканата</option><option value=\"curator\">Куратор</option></select>");
    div.insertAdjacentHTML("beforeend", "<input type=\"text\" placeholder=\"Значение\" id=\"inputString\" name=\"property\" required>");
    div.insertAdjacentHTML("beforeend", "<button type=\"button\" id=\"removeAddInfo\" onclick=\"removeNewProperties()\">Удалить поле</button>");
...
function removeNewProperties() {
    let div = document.getElementById("mainInfo");
    div.insertAdjacentHTML("beforeend", "<button type=\"button\" id=\"addInfo\" onclick=\"addNewProperties()\">Другая информация</button>");
    let key = document.getElementById("addInfo");
    let inputString = document.getElementById("inputString");
    let removeAddInfo = document.getElementById("removeAddInfo");
    div.removeChild(key);
    div.removeChild(inputString);
    div.removeChild(removeAddInfo);
}
